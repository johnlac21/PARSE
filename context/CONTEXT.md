...
- **Tables**: `Project` (id TEXT PK, name, task_modality, created_at, config JSON). `Prompt` (id INTEGER PK, project_id FK, prompt_id TEXT logical id, prompt_text, variant TEXT, features_applied JSON, metadata_ JSON). `Run` (id TEXT PK, project_id FK, model_provider, model_id, system_prompt, temperature, status, started_at, completed_at, total_prompts, completed_prompts, error_count, progress_message). `Result` (id INTEGER PK, run_id FK, prompt_id FK → **Prompt.id** not logical prompt_id, raw_response, parsed_label, parsed_index, is_valid INT 0/1, error_message, latency_ms, created_at). `UploadStaging` (project_id PK, row_index PK, data JSON); FK to projects with ondelete=CASCADE. Note: Result columns are Likert-shaped (`parsed_label`, `parsed_index`, `is_valid`). For non-Likert modalities, additional parser fields (e.g. `titles` from `RecommendationListParser`) are not persisted. Re-enabling other modalities will require either extending the schema (e.g. a `parsed_data` JSON column) or repurposing existing columns. See DEVLOG entry "Pre-GitHub cleanup; v1 lockdown".
...
- **recommendation_list** *(scaffolded; not user-selectable in v1)*: Parser returns `titles`, `count`, `is_valid`; runner still only writes parsed_label/parsed_index/is_valid (so parsed_index typically unused; is_valid = len(titles)>0).
- **free_text** *(scaffolded; not user-selectable in v1)*: Parser returns `text`, `word_count`, `is_valid`, `is_refusal`; only `is_valid` is stored in Result. Completeness analysis uses `raw_response` for refusal detection and uses `parsed_index.notna()` or `is_valid==1` for valid count.
...
- **Run selection**: `selectedRunIds` lives in Zustand (`project-store`). Not persisted to URL or localStorage; resets when store is reset (e.g. project change). **Results page** uses derived `effectiveRunIds` (see `context/DEVLOG.md`, session "Pre-GitHub cleanup; v1 Likert lockdown"): if the user has not explicitly selected runs, all completed runs are included; the store’s `selectedRunIds` is updated only when the user checks/unchecks runs. Fetches completeness/difference/directional when `projectId` or `effectiveRunIds` changes.
